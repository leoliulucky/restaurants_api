...
public class MobileVerifyCode
{
	 ...
	 * 日志对象
	 */
	private static Logger logger = LoggerFactory.getLogger(MobileVerifyCode.class);
	private final static int OFFSET = 538309;
	private final static long EXPIRED = 30 * 60 * 3000; // 失效时间（30分钟）
	private final static long FREQ = 1 * 60 * 1000; // 同一用途重复发送频率（1分钟）
	...
	public static String generateMailValidCode()
	{
		...
		try {
			...
			logger.error("随机生成十位数ValidCode码错误", e);
		}
